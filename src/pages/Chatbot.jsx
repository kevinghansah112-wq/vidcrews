...
const SUPABASE_URL = 'https://ihgyuvmgxooqpxrkghbx.supabase.co'
...
Your job is to:- Help users navigate the platform- Answer questions about how VidCrews works- Give advice on creative production, rates, equipment, and bookings- Help clients describe what they're looking for in a creative- Help creatives with their profiles, rates, and how to get booked- Be knowledgeable about the African creative industry (Ghana, Nigeria, Kenya, South Africa, etc.)
Keep responses concise, friendly, and practical. You can use occasional African slang like "chale" or "omo" to feel local. Never make up specific user data.`
export default function Chatbot({ go }) {
  const { profile } = useAuth()
  const [messages, setMessages] = useState([
    { role: 'assistant', content: `Hey${profile ? ' ' + (profile.full_name?.split(' ')[0] || profile.username) : ''}! I am VidBot, your creative industry assistant. Ask me anything about VidCrews, production rates, equipment, or finding the right crew. What is on your mind?` }
  ])
  ...
  const send = async () => {
    ...
    try {
      ...
      const reply = data.content?.[0]?.text || "Sorry, I could not process that. Try again?"
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column', height: 560 }}>
        <div style={{ flex: 1, overflowY: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {loading && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: 'var(--accent-fg)' }}>AI</div>
          )}
        ...
        {messages.length === 1 && (
          <div style={{ padding: '0 20px 12px', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {['What is VidCrews?', 'How do I get booked?', 'What should my day rate be?', 'How do I write a good bio?'].map(q => (
        ...
        <div style={{ padding: '12px 16px', borderTop: '1px solid var(--border)', display: 'flex', gap: 8 }}>
          ...
          />
          <button className="btn" onClick={send} disabled={!input.trim() || loading} style={{ padding: '9px 16px', flexShrink: 0 }}>
            Send
          </button>
        </div>
      </div>
